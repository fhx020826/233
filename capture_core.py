...
from peer_match_test import http
from util.packagepretreat import dict2strlist,get_raw,bytes2str_dict
...
class Core():
    ...
    temp_file = None
    # 计数器
    counter = {"ipv4": 0, "ipv6": 0, "tcp": 0, "udp": 0, "icmp": 0, "arp": 0,"bittorrent":0,"http_tracker":0,"bt_dht":0,"udp_tracker":0,"utp":0}
    logger = get_logger('D:\\c\\magnet-dht-master\\magnet_dht\\pybt\\log\\scrape.log')
        ...
        try:
            ...
            if not self.pause_flag and packet.name == "Ethernet":
                ...
                if ether_type == "IP":
                    ...
                    self.counter["ipv4"] += 1
                    #bt分析
                    result,head,kind = self.analizy(packet,self.packet_id)
                    # print(self.packet_id,kind)
                    if kind == 'bittorrent':
                        self.isbt[(source,destination)] = True
                        self.finresult[self.packet_id] = result
                    if (source,destination) in self.isbt and self.isbt[(source,destination)] and (not result) and get_raw(packet) and len(get_raw(packet))>500:
                        kind = 'bittorrent'
                        self.finresult[self.packet_id] = get_raw(packet).hex()
                    #print(self.packet_id,head,result)
                # IPv6
    ...
    def on_click_item(self, this_id):
        ...
        """
        self.logger.info(f'查看数据包 {this_id} 的数据解析')
        try:
            ...
            first_temp, second_temp = self.get_next_layer(packet,this_id)
    ...
    def get_next_layer(self, packet,this_id):
        """
        递归处理下一层信息
        ...
        try:
            ...
            # 第四层: 传输层
                ...
                #TODO 进行http Tracker的检查，如果没有，则进入应用层
                result = http(packet)
                #print('http',result)
                if result:
                    next_layer = dict2strlist(result)
                    #print(next_layer)
                    first_return.append(f"HTTP Tracker")
                    second_return.append(next_layer.copy())
                    next_layer.clear()
                    self.counter['http_tracker'] += 1
                if this_id in self.finresult:
                    result = self.finresult[this_id]
                    if result:#bt检测
                        if isinstance(result,list):
                            for i in result:
                                first_return.append(f"BitTorrent")
                                next_layer = dict2strlist(i)
                                second_return.append(next_layer.copy())
                                next_layer.clear()
                        else:
                            first_return.append(f"BitTorrent")
                            next_layer.append(f'Data: {self.finresult[this_id]}')
                            second_return.append(next_layer.copy())
                            next_layer.clear()
                ...
                if length > 0:
                    ...
                    #TODO
                        ...
                        first_return.append(f"Data: {length} Bytes")
                        ...
                        if result:
                            next_layer = dict2strlist(result)
                            first_return.append(f"UDP Tracker Protocol")
                            second_return.append(next_layer.copy())
                            next_layer.clear()
                            self.counter['udp_tracker'] += 1
                        else:#udp分析
                            result = krcp(data)
                            if result:#dht

                                next_layer = dict2strlist(result)
                                first_return.append(f"Bittorrent DHT Protocol")
                                second_return.append(next_layer.copy())
                                next_layer.clear()
                                self.counter['bt_dht'] += 1
                            else:#utp
                                predict, utp_len = utp_partion(data)
                                if predict:
                                    next_layer = dict2strlist(predict)
                                    first_return.append(f"uTorrent Transport Protocol")
                                    second_return.append(next_layer.copy())
                                    next_layer.clear()
                                    if predict['Type'] == '0':#bt
                                        result = self.finresult[this_id]
                                        if result:#bt检测
                                            if isinstance(result,list):
                                                for i in result:
                        ...
                        #     first_return.append("uTorrent Transport Protocol")
                        #     for i in range(len(result)):
                        #         next_layer.append(f'{keys[i]}: {values[i]}')
                        # result,utp_len = utp_partion(payload)
                        # print('result',result,utp_len)
                        # if result :
                        #     keys = list(result.keys())
                        #     values = list(result.values())
                        #     first_return.append("uTorrent Transport Protocol")
            ...
            first_temp, second_temp = self.get_next_layer(packet.payload,this_id)
            first_return += first_temp
            second_return += second_temp
        except:
            # 未知数据包
    ...
    def open_pcap_file(self):
        ...
        self.logger.info(f'打开文件： {filename}')
        sniff(
    ...
    def get_app_count(self):
        ...
        获取应用层bt数据包的数量
        """
        the_keys = ["bittorrent","http_tracker","bt_dht","udp_tracker","utp"]
        counter_copy = self.counter.copy()
        return_dict = {}
        for key, value in counter_copy.items():
            if key in the_keys:
    ...
    def tcp(self,data: bytes, i: int, segment_tuple: tuple):
        isbt = False
        #print(data)
        if data:
            ##print('bt')
            if segment_tuple in self.segment_dict:  # 存在前缀截断
                data = self.segment_dict[segment_tuple]+data
                # if i == 1758:
                #     ##print(len(data),data.hex())
                # #del self.segment_dict[segment_tuple]
                dictlist, segment, isbt = bt_partion(data)
                
                self.head[i] = self.flag[segment_tuple]  # 当前端的头记录
                if dictlist:  # 如果多个截断拼接完成，删除该方向的头
                    del self.flag[segment_tuple]
                    del self.segment_dict[segment_tuple]
            else:
                dictlist, segment, isbt = bt_partion(data)
            if segment:
                self.segment_dict[segment_tuple] = segment
                # 定义头的位置
                if segment_tuple not in self.flag:
                    self.flag[segment_tuple] = i  # 记录特定方向的头
                    self.head[i] = i  # 记录头的位置
            # ##print(self.head[i])
            # ##print(dictlist)
    ...
    def udp(self,data: bytes, i: int, segment_tuple):
        if data:
            dictlist = []
            ...
            if prodict:
                ...
                if prodict['Type'] == '0':
                    kind = 'bittorrent'
                    if segment_tuple in self.segment_dict:  # 存在前缀截断
                        data = self.segment_dict[segment_tuple]+data[utp_len:]
    ...
    def analizy(self,package, i: int) -> list:

        kind = None
        self.head.append(-1)  # 当前包的头为自己，标记为-1
        result = http(package)
        ##print('http',result)
        if result:#tracker
            kind = 'http_tracker'
        ...
        if package.haslayer("UDP"):
            ...
            if result:
                kind = "udp_tracker"
                return result,-1,kind
            result,kind = self.udp(data, i, segment_tuple)
            ##print('udp',result)
        elif package.haslayer("TCP"):
            result,isbt = self.tcp(data, i, segment_tuple)
            if isbt or result:
                kind = 'bittorrent'
        return result,self.head[i],kind
